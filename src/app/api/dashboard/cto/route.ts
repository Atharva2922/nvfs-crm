...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!ExecutiveDashboardService.isCtoAuthorized(user)) {
      return errorResponse("Forbidden: Access restricted to CTO & Engineering leadership", "FORBIDDEN", 403);
    }
    ...
    const data = await ExecutiveDashboardService.getCtoDashboardData(user, {
      dateRange,
      startDate,
      endDate,
    });
    ...
    console.error("[CTO Dashboard API Error]:", error);
    return errorResponse(error.message || "Failed to load CTO dashboard data", "INTERNAL_ERROR", 500);
  }
}
