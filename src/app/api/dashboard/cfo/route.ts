...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!ExecutiveDashboardService.isCfoAuthorized(user)) {
      return errorResponse("Forbidden: Access restricted to CFO & Financial leadership", "FORBIDDEN", 403);
    }
    ...
    const data = await ExecutiveDashboardService.getCfoDashboardData(user, {
      dateRange,
      startDate,
      endDate,
    });
    ...
    return errorResponse(error.message || "Failed to load CFO dashboard data", "INTERNAL_ERROR", 500);
  }
}
